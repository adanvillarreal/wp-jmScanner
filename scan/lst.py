...
def sitecheck():
	...
	f.write ("http://"+ url.netloc + url.path + "\n")
	f.close()
	...
	f.write ("Error Page: http://"+ url.netloc + url.path + "\n")
	f.close()
	  ...
	  try:
	      ...
	      url = urlparse(url)
 	      print url
	      conn = httplib.HTTPConnection(url.netloc + url.path) #agregadp
	      site = "http://" + url.netloc + url.path
		...
		if count_wp_modules == 0 and count_jm_modules == 0 :
		  ...
		  if html != found:
		    notid_wordpress_write()
		    sys.stdout.write(colors.color.YELLOW +"  [?] "+ url.netloc + url.path + " seems to be Wordpress based..["+str(wppersent)+"%]\n" + colors.color.RESET)
		    sys.stdout.flush()

		  else:
		    notid_joomla_write()
		    sys.stdout.write(colors.color.YELLOW + "  [?] "+ url.netloc + url.path + " seems to be Joomla based..["+str(jmpersent)+"%]\n" + colors.color.RESET)
		    sys.stdout.flush()
